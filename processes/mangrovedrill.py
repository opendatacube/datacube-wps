...
def _processData(datas, **kwargs):
    data = datas[0]
    woodland = data.where( data == 1 ).count(['x', 'y']).rename(name_dict={'mangrove_cover': 'woodland'})
    open_forest = data.where( data == 2).count(['x', 'y']).rename(name_dict={'mangrove_cover': 'open_forest'})
    closed_forest = data.where( data == 3).count(['x', 'y']).rename(name_dict={ "mangrove_cover": 'closed_forest'})
